...
import { Upload, X, Camera, Gem, MapPin, Award, DollarSign, FileText, Palette, AlertCircle, CheckCircle } from 'lucide-react';
...
const CreateGemstoneForm = () => {
  ...
  const cutTypes = [
    ...
    "Asscher", "Marquise", "Heart", "Pear", "Cabochon", "Raw/Rough", "Other"
  ];

  const certifyingBodies = [
    "None", "GIA", "AIGS", "Gübelin", "SSEF", "GRS", 
    "Lotus", "EGL", "AGS", "Other"
  ];
  ...
  const colorOptions = [
    ...
    { name: 'White', value: 'white', color: 'bg-gray-100 border-2 border-gray-300' },
    { name: 'Clear', value: 'clear', color: 'bg-gradient-to-br from-gray-100 to-gray-200 border-2 border-gray-300' }
  ];
  ...
  const handleSubmit = async (isDraft = false) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setSubmitMessage(isDraft ? 'Gemstone saved as draft successfully!' : 'Gemstone submitted for review successfully!');
      ...
      if (error.errors && Array.isArray(error.errors)) {
        ...
        setSubmitMessage(error.message || 'Error submitting gemstone. Please try again.');
        toast.error(error.message || 'Failed to submit gemstone. Please try again.');
      }
    } finally {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="md:col-span-2">
                ...
                <div className="flex justify-between items-center mt-2">
                  ...
                  <div className={`w-full max-w-xs bg-gray-200 rounded-full h-2 ml-4 ${formData.description.length > 800 ? 'bg-red-200' : ''}`}>
                    <div 
                      className={`h-2 rounded-full transition-all duration-300 ${formData.description.length > 800 ? 'bg-red-500' : 'bg-blue-500'}`}
            ...
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-2xl p-6 border border-blue-100">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  <h4 className="text-sm font-semibold text-gray-700">Dimensions (Optional)</h4>
              ...
              <div className="grid grid-cols-5 md:grid-cols-10 gap-3 mb-4">
                {colorOptions.map((color) => (
                  <button
                    ...
                    className={`w-12 h-12 rounded-full ${color.color} border-4 transition-all duration-200 hover:scale-110 focus:outline-none focus:ring-4 focus:ring-blue-200 ${
                      formData.color === color.value ? 'border-blue-600 ring-4 ring-blue-200' : 'border-gray-300'
                    }`}
                    title={color.name}
                  />
                ))}
            ...
            <div className="bg-gradient-to-r from-yellow-50 to-orange-50 rounded-2xl p-6 border border-yellow-100">
              ...
              <div className="mb-6">
                <div className="border-3 border-dashed border-yellow-300 rounded-2xl p-8 text-center hover:border-yellow-400 hover:bg-yellow-25 transition-all duration-300 cursor-pointer group">
                  ...
                  <p className="text-yellow-600 text-sm mt-2">PNG, JPG, GIF up to 10MB each • Upload multiple angles for best results</p>
                </div>
              </div>
          ...
          <div className="space-y-6">
            <div className="bg-gradient-to-r from-emerald-50 to-cyan-50 rounded-2xl p-6 border border-emerald-100">
              ...
              <div className="mt-6 p-4 bg-white rounded-xl border border-gray-200">
                <h4 className="text-sm font-semibold text-gray-700 mb-3">Pricing Guidelines</h4>
                <div className="space-y-2 text-sm text-gray-600">
                  <p>• <span className="font-medium">Minimum Bid:</span> Starting price for the auction</p>
                  <p>• <span className="font-medium">Reserve Price:</span> Minimum price you'll accept (hidden from bidders)</p>
                  <p>• <span className="font-medium">Tip:</span> Research similar gemstones to set competitive prices</p>
                </div>
              </div>
            </div>
          </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="text-center mb-12">
          ...
          <h1 className="text-4xl font-bold text-gray-900 mb-2">Create Gemstone Listing</h1>
          <p className="text-xl text-gray-600">Showcase your precious gemstone to the world</p>
        </div>
          ...
          <div className={`mb-6 p-6 rounded-2xl border-2 shadow-lg ${
            ...
            <div className="text-center">
              ...
              <h3 className={`text-2xl font-bold mb-2 ${
                ...
                {submitStatus === 'success' ? '🎉 Gemstone Added Successfully!' : '❌ Error Occurred'}
              </h3>
            ...
            {steps.map((step, index) => {
              ...
              return (
                <div key={step.id} className="flex items-center">
                  ...
                  >
                    <Icon className={`h-6 w-6 mb-2 ${isActive ? 'text-white' : isCompleted ? 'text-green-600' : 'text-gray-400'}`} />
                    <span className={`text-sm font-semibold ${isActive ? 'text-white' : isCompleted ? 'text-green-700' : 'text-gray-600'}`}>
                      {step.name}
                    </span>
                    {(isCompleted || (currentStep === step.id && isValid)) && (
                      <div className={`w-2 h-2 rounded-full mt-1 ${isActive ? 'bg-white' : 'bg-green-500'}`} />
                    )}
                  </button>
                  {index < steps.length - 1 && (
                    <div className={`w-8 h-1 mx-2 rounded-full ${isCompleted ? 'bg-green-400' : 'bg-gray-200'}`} />
                  )}
                </div>
              );
            })}
        ...
        <div className="mt-12 bg-white rounded-2xl p-6 shadow-lg border border-gray-100">
          <h3 className="text-lg font-semibold text-gray-800 mb-4">💡 Tips for a Great Listing</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="text-center">
              <div className="w-12 h-12 bg-blue-100 rounded-full flex items-center justify-center mx-auto mb-3">
                <Camera className="h-6 w-6 text-blue-600" />
              </div>
              <h4 className="font-semibold text-gray-800 mb-2">High-Quality Photos</h4>
              <p className="text-sm text-gray-600">Upload clear, well-lit images from multiple angles</p>
            </div>
            <div className="text-center">
              ...
              <p className="text-sm text-gray-600">Include all relevant details about your gemstone</p>
            </div>
            <div className="text-center">
              <div className="w-12 h-12 bg-purple-100 rounded-full flex items-center justify-center mx-auto mb-3">
                <Award className="h-6 w-6 text-purple-600" />
              </div>
              <h4 className="font-semibold text-gray-800 mb-2">Certificates Matter</h4>
              <p className="text-sm text-gray-600">Professional certificates increase buyer confidence</p>
            </div>
          </div>
        </div>
      </div>

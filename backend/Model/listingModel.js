...
const gemstoneSchema = new Schema({
  ...
  clarity: { 
    type: String, 
    enum: {
      values: ["FL","IF","VVS1","VVS2","VS1","VS2","SI1","SI2","I1","I2","I3","Not Graded"],
  ...
  origin: {
    country: { 
      ...
      validate: {
        ...
        message: "Country name should only contain letters, spaces, and hyphens"
      }
    },
    region: { 
      type: String, 
  ...
  certificateDetails: {
    ...
    certificateNumber: { 
      ...
      validate: {
        ...
        message: "Certificate number is required when certificate is available"
      }
    },
    certifyingBody: { 
      type: String, 
      enum: {
        values: ["GIA","AIGS","Gübelin","SSEF","GRS","Lotus","EGL","AGS","Other","None"],
...
/* ====================== INSTANCE METHODS ====================== */
gemstoneSchema.methods.updateVerificationStatus = function(status, notes = "", adminId = null) {
...
gemstoneSchema.statics.findInPriceRange = function(minPrice, maxPrice, limit = 50) {
  return this.find({
  ...
  return this.aggregate([
    {
      $group: {
        ...
        pending: { $sum: { $cond: [{ $eq: ["$verificationStatus", "submitted"] }, 1, 0] } },
...
gemstoneSchema.pre('save', function(next) {
  ...
  if (this.isModified('name') || this.isModified('color') || this.isModified('category')) {
    this.tags = [

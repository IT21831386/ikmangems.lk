...
export const validateGemstone = [
  ...
  body("minimumBid")
    .notEmpty()
    .withMessage("Minimum bid is required")
    .isNumeric()
    .withMessage("Minimum bid must be a valid number")
    .isFloat({ min: 1 })
    .withMessage("Minimum bid must be at least 1 LKR")
    .toFloat(),
  ...
  body("weight")
    ...
    .withMessage("Weight must be between 0.01 and 10,000 carats")
    .toFloat(),
  ...
  body("color")
    ...
    .withMessage("Color description must be between 2-50 characters")
    .matches(/^[a-zA-Z\s\-]+$/)
    .withMessage("Color should only contain letters, spaces, and hyphens")
    .trim(),
  ...
  body("origin")
    .optional()
    .isLength({ min: 2, max: 50 })
    .withMessage("Origin must be between 2-50 characters")
    .trim(),
...
export async function createGemstone(req, res) {
  try {
    ...
    const { name, description, category, minimumBid, origin, color, weight } =
      req.body;
    ...
    const [gemstones, total] = await Promise.all([
      Gemstone.find(query)
        ...
        .select(
          req.user?.role === "admin"
            ? {}
            : "-documentValidation -sellerInfo.phone"
        ),
...
export async function getGemstone(req, res) {
  try {
    ...
    if (
      ...
      gemstone.sellerId.toString() !== req.user.id
    ) {
      return res.status(403).json({
        success: false,
...
export async function updateGemstone(req, res) {
  try {
    ...
    if (
      ...
      statusMessage +=
        ". Your listing will be reviewed again due to content changes.";
...
export async function deleteGemstone(req, res) {
  try {
    ...
    const isPermanent =
      req.query.permanent === "true" && req.user?.role === "admin";

    if (isPermanent) {
      // Permanent deletion (admin only)
      await Promise.all(
        gemstone.images.map((img) =>
          fs
            .unlink(img.url)
            .catch((err) =>
